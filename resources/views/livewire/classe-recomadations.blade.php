<div class=" bg-white dark:bg-gray-900 m-2 rounded-lg shadow-md p-6 space-y-4">
    @forelse ($etuds as $index => $etud)
        <div class="flex flex-col space-y-1 col-span-2 w-full ">
            <div class="flex justify-between">
            <label for="note"  class="font-semibold text-sm mx-1 text-gray-900 dark:text-gray-100">{{ $etud->nb }} - {{ app()->getLocale() == 'ar' ? $etud->nom : $etud->nomfr }}:</label>
            </div>
            <textarea wire:model='notes.{{ $index }}.note' class="textearea h-24 w-full @error('note') reds @enderror" type="text" required  ></textarea>
        </div>
    @empty
    @endforelse

    <div class='px-2  flex space-x-3 justify-end items-center w-full'>
