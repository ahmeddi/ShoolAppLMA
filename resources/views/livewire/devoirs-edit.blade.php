                ...
                <div class="grid lg:grid-cols-2 gap-x-6 gap-y-3 px-12  justify-items-center sm:grid-cols-1 ">
                    <div class="flex flex-col space-y-1">
                        <div class="flex justify-between">
                          <label  class="labels"> {{ __('devoirs.nom') }}:</label>
                    ...
                    <div class="flex flex-col space-y-1">
                        <div class="flex justify-between">
                          <label  class="labels"> {{ __('devoirs.nomfr') }}:</label>
                    ...
                    <div class="flex flex-col space-y-1 col-span-2 w-full">
                        <div class="flex justify-between">
                          <label  class="labels"> {{ __('devoirs.trim') }}:</label>
                          @error('sem') <span class="danger">{{ $message }}</span> @enderror  
                        </div>
                          <select  wire:model.defer="sem"   class="inputs w-full @error('sem') reds @enderror">
                            <option >-----</option>
                                @foreach ($sems as $sem)
                                <option  value="{{ $sem->id }}"> 
                                    @if ( app()->getLocale() == 'ar')
                    ...
                    @endforeach 
                      ...
                      {{-- <div class=" flex space-x-2 items-center">
                            <input id="dev" wire:model.defer='devoir' type="checkbox" class="check" >
                            <label for="dev"  class="flex items-center  w-40">
