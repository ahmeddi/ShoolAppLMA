<div class="w-full h-full px-2  ">
    <div class= "flex justify-between my-3 ">
        <button wire:click="$dispatch('open')" class='focus:outline-none px-4 py-2 dark:text-gray-900 dark:bg-gray-100 text-white rounded-md hover:outline-none bg-teal-600 hover:bg-teal-800' >
            {{ __('devoirs.devoir-add') }}
        </button>
        <div>
            <select wire:model.live="sem_id" wire:change='updateSems'  class="inputs w-full @error('exam') reds @enderror">
                <option value="*">-----</option>
                    @foreach ($sems as $sem)
                        <option  value="{{ $sem->id }}"> 
                            @if ( app()->getLocale() == 'ar')
    ...
    <table class="w-full divide-y divide-gray-200 dark:divide-gray-600 shadow-md rounded-t-xl overflow-hidden">
                ...
                @foreach ($devs as $dev)
                    <tr class="h-5 ">
                            ...
                            <div class="flex items-center">
                                <div class="ml-2 flex space-x-2 ">
                                    <div class="flex flex-col">
                                        <div class="text-sm font-devibold text-gray-900 dark:text-gray-200">
                                                 @if ( app()->getLocale() == 'ar')
                                                    {{ $dev->nom }}
                                                @else
                                                    {{ $dev->nomfr }} 
                        ...
                        <td class="flex space-x-2 py-2 h-full items-center">
                            <div class="mx-4">
                                <button wire:click="$dispatch('edit',{ id: {{ $dev->id }} })" >
                            ...
                            <div>
                                <button  wire:click="$dispatch('dtls',{ id: {{ $dev->id }} })" class=" stroke-red-600">
